...
async def vehicle_polling(app: FastAPI):
    while True:
        try:
            ...
            raw_data = await app.state.at_client.get_combined_feed()
            transformed_data = transformers.transformer.parse_realtime_data(raw_data)
            
            print("[POLLER] Updating database")
            await app.state.repository.upsert_realtime_data(transformed_data)
            ...
            await app.state.repository.clean_realtime_data()
